...
(function($){
    ...
    var trunParamConfig2RequestParamStr = function(json, settings){
        ...
        for(key in json){
        	if(key == 'pageNo'){
        		pageNo = json[key];
        		str += "&pageInfo.page="+json[key];
        	}else if(key == 'pageSize'){
        		pageSize = json[key];
        		str += "&pageInfo.rows="+json[key];
        	}else if(str==""){
                str += key+"="+json[key];
            }else{
                str += "&"+key+"="+json[key];
    ...
    var fetchData = function(pageNo,isToPage, settings){  
        ...
        $.ajax({
            ...
            success :function(datas){
                ...
                settings.buildHtml_fun(jsonData.gridModel,jsonData.records);
    ...
    var bindEvent = function(settings){
        ...
        var but = settings.pageWidget.find("button[name='seach']");
        $(but).click(function(){
        	settings.seachNum = settings.pageWidget.find('.seachNum').val();
        	if(settings.seachNum != "" && settings.seachNum > 0) {
        		trunTargetPage(settings.seachNum, false, settings);
        	} else {
        		settings.seachNum = "";
        	}
        });
        settings.pageWidget.find('.seachNum').bind('keypress',function(event){
            if(event.keyCode == "13") {
            	settings.seachNum = settings.pageWidget.find('.seachNum').val();
            	if(settings.seachNum != "" && settings.seachNum > 0) {
            		trunTargetPage(settings.seachNum, false, settings);
    ...
    var buildPageWidget = function(page,settings){
        ...
        /* --------------- 下面开始进入真正的分页控件构建过程 -------------- */ 
        settings.pageWidget.css("padding-top","10px");
        settings.pageWidget.append("<div class='pull-right' style=' line-height:32px;'>共有 "+page.getTotalData()+" 条数据，当前第 "+pageNo+" 页</div>");
        settings.pageWidget.append("<div class='pull-left'>");
        settings.pageWidget.find('div').next().append("<span class='pagination-centered pull-left'><ul class='pagination'></ul></span>");
        ...
        /* --------------- 4.结束：构建“下一页”和“尾页”控件 ------------- */  
        if(settings.seachNum != "") {
        	settings.pageWidget.find("ul").append("<li><input class='form-control w60 pull-left seachNum' type='text' value='"+settings.seachNum+"'><button class='mar btn' name='seach'>跳转</button></li>");
        } else {
        	settings.pageWidget.find("ul").append("<li><input class='form-control w60 pull-left seachNum' type='text'><button class='mar btn' id='buttonseach' name='seach'>跳转</button></li>");
        }
        settings.pageWidget.find('div').next().append('<span class="btn-group dropup pull-left" ></span>');
        /* --------------- 5构建 ------------- */
