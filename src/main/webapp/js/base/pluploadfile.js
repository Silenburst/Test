//plupload 组件的相对根路径
var plupload_basePath="../js/plupload";
   ...
   init: {
       ...
       FilesAdded: function(up, files) {
       	...
       	if(imgType==1){
       		...
       		$imgContainer = $("#ulHouseExclusiveFiles2");
       	}
       	    	
       	plupload.each(files,function(file) {
			var sHtml = ' <div class="col-lg-6 col-xs-12 form-group"><img id="file-'
					+ file.id
					+ '" width="100" height="50" class="img-thumbnail"></div>';
       ...
       FileUploaded : function(up, file, resp) {
			...
			var theForm = $("#formWebImageofhouse #webImageofhouse_image_hidden");
			var topIndex = $("#houseExclusiveImgType").val();
//			$("#tupianming").val(result.fileName);
			$("#houseExclusiveImgType").attr("disabled","");
			var sHtml = '<input type="hidden" id="txtExclusiveImgFileName' + houseExclusiveImgIndex+ '" name="condition.imageUrl['+houseExclusiveImgIndex+'].img" value="'
			+ result.fileName + '"/><input type="hidden" id="txtIndex'+houseExclusiveImgIndex+'" name="condition.imageUrl['+houseExclusiveImgIndex+'].statuss" value="1">'
			+ '<input type="hidden" id="txtType' + houseExclusiveImgIndex
			+ '" name="condition.imageUrl['+houseExclusiveImgIndex+'].type" value="'
		...
		//	theImageLi.append('<a href="#" class="name"><span>'+result.fileName+'</span></a>');
			theImageLi.append('<div class="image-options"><a href="javascript:void(0)" onclick="removeExclusiveImg('
