package com.newenv.lpzd.spider.action;
...
import com.newenv.lpzd.spider.service.SpiderService;
...
public class SpiderAction extends BaseAction
{
	...
	private SpiderCoummunityEntity spiderCoummunityEntity;
	...
	public String getQyValue()
	{
		return qyValue;
	}

	public void setQyValue(String qyValue)
	{
	...
	public SpiderCoummunityEntity getSpiderCoummunityEntity()
	{
		return spiderCoummunityEntity;
	}

	public void setSpiderCoummunityEntity(SpiderCoummunityEntity spiderCoummunityEntity)
	{
		this.spiderCoummunityEntity = spiderCoummunityEntity;
	}
	...
	private String from = ""; // 从哪个列表页面过来的 lpxx: lpxxList, apply: 楼盘采集页面,
								// check：楼盘字典审核页面
	 ...
	 * @return
	 */
	public String doGather()
	{
		try
		{
			int floorPlatformId=0;
			
			// 查询小区具体信息.
			HashMap<String,String> hm=new HashMap<String,String>();
			hm.put("lpName", lpname);
			hm.put("cityValue", cityValue);
			Object[] objs=spiderService.queryCommunicationInfo(hm);
			if(objs!=null && objs.length>0)
			{
				...
				qyId=(int)objs[3];
				sqId=(int)objs[4];
				floorPlatformId=(int)objs[5];
			}
			
			if(floorPlatformId>0)
			{
				lpxx=xhjLpxxService.getById(floorPlatformId);
				String moreText=lpxx.getMore();
				moreText=moreText.replaceAll("\r\n", " ");
				moreText=moreText.replaceAll("\n", " ");
				lpxx.setMore(moreText);
			}
			...
			//pageInfo = spiderService.queryData(spiderCoummunityEntity, pageInfo);
	...
	public String queryGatherData()
	{
		try
		{
			...
			pageInfo = spiderService.queryGatherData(spiderCoummunityEntity, pageInfo);
			return jsonAjaxResult(JSON.toJSONString(pageInfo));
		}
		catch (Exception e)
		{
	 ...
	 * @return
	 */
	public int updateCrawlStatus()
	{
		try
		{
			return spiderService.updateCrawlStatus(idList);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public String imageQuery()
	{
		try
		{
			...
			pageInfo = spiderService.imageQuery(imgUrl, imgType, pageInfo);
			return jsonAjaxResult(JSON.toJSONString(pageInfo));
		}
		catch (Exception e)
		{
	...
	{
		return spiderService.queryProvinceId(couId,provinceValue);
	}
	...
	{
		return spiderService.queryProvinceId(proId,cityValue);
	}
	...
	{
		return spiderService.queryProvinceId(cityId,qyValue);
	}
	...
	{
		return spiderService.queryProvinceId(proId,sqValue);
	}
	...
	public String showDetail2(){
		...
		Double cs=0.0;
		if(cssCount>0.0){
			 cs = (csbCount-cssCount) / cssCount * 100;//在售房环比上月数
		}
		Double cz=0.0;
		if(czsCount>0.0){
			 cz = (czbCount-czsCount) / czsCount * 100;//环比上月租数
		}
		Double kz=0.0;
		if(kzsCount>0.0){
			kz = (kzbCount-kzsCount) / kzsCount * 100;//环比上月空置数
		}
		Double cscj=0.0;
		if(cssCount>0.0){
			cscj = (csbcjCount-csscjCount) / csscjCount * 100;//在售成交房环比上月数
		}
		Double czcj=0.0;
		if(cssCount>0.0){
			czcj = (czbcjCount-czscjCount) / czscjCount * 100;//在租成交房环比上月数
		}
		
		Double csjj=0.0;
		if(cssCount>0.0){
			csjj = (csbjjCount-cssjjCount) / cssjjCount * 100;//在售均价环比上月数
		}
		Double czjj=0.0;
		if(cssCount>0.0){
			czjj = (czbjjCount-czsjjCount) / czsjjCount * 100;//在租均价环比上月数
		}
		list.add(fhCount.get(0));//小区在售总数
		list.add(fhCount.get(1));//小区内部系统在售数
		list.add(fhCount.get(2));//小区外部系统在售数
		list.add(df.format(cs));//在售房环比上月数
		...
		list.add(czscjCount);
		list.add(czcj);
		list.add(byjujiaCount.get(0));
		list.add(byjujiaCount.get(1));
		list.add(csjj);
		list.add(czjj);
		this.bindParam("loadLpxx", loadLpxx);
	...
	public String lpxxlptp(){
		...
		return jsonAjaxResult(JSON.toJSONString(lptp));
	}
...
//			lpxx.setLtype(Integer.parseInt(lpLtype));
	...
	public String addDong(){
		...
		try {
			...
			String prefix = this.getRequest().getParameter("prefix");
			String start = this.getRequest().getParameter("start");
			String end = this.getRequest().getParameter("end");
			String suffix = this.getRequest().getParameter("suffix");
			String type = this.getRequest().getParameter("type");
			...
			XhjLpdong xhjLpdong = new XhjLpdong();
			if(lpid!=null&&!"".equals(lpid)){
				xhjLpdong.setLpId(Integer.parseInt(lpid));
			}
			xhjLpdong.setLpdName(lpdName);
			xhjLpdong.setPrefix(prefix);
			xhjLpdong.setStart(start);
			xhjLpdong.setEnd(end);
			xhjLpdong.setSuffix(suffix);
			xhjLpdong.setType(type);
			if(dType!=null&&!"".equals(dType)){
				xhjLpdong.setDtype(Integer.parseInt(dType));
			}
			if(dyNum!=null&&!"".equals(dyNum)){
				xhjLpdong.setDyNum(Integer.parseInt(dyNum));
			}
			xhjLpdong.setAges(ages);
			xhjLpdong.setSize(size);
			if(topNum!=null&&!"".equals(topNum)){
				xhjLpdong.setTopNum(Integer.parseInt(topNum));
			}
			if(underNum!=null&&!"".equals(underNum)){
				xhjLpdong.setUnderNum(Integer.parseInt(underNum));
			}
			if(located!=null&&!"".equals(located)){
				xhjLpdong.setLocated(Integer.parseInt(located));
			}
			if(ownership!=null&&!"".equals(ownership)){
				xhjLpdong.setOwnership(Integer.parseInt(ownership));
			}
				xhjLpdong.setRemarks(remarks);
	...
	public String addDany(){
		...
		try {
			...
			String dyName = this.getRequest().getParameter("dyName");
			String prefix = this.getRequest().getParameter("prefix");
			String start = this.getRequest().getParameter("start");
			String end = this.getRequest().getParameter("end");
			String suffix = this.getRequest().getParameter("suffix");
			String type = this.getRequest().getParameter("type");
			String lpid = this.getRequest().getParameter("lpid");
			String dType1 = this.getRequest().getParameter("dType1");
			String topNum1 = this.getRequest().getParameter("topNum1");
			String underNum1 = this.getRequest().getParameter("underNum1");
			String totalNum = this.getRequest().getParameter("totalNum");
			String dihu = this.getRequest().getParameter("dihu");
			String remarks1 = this.getRequest().getParameter("remarks1");
			int userid = SecurityUserHolder.getCurrentUserLogin().getUserProfile().getId();
			XhjLpdanyuan xhjLpdanyuan = new XhjLpdanyuan();
			
			if(dzid!=null&&!"".equals(dzid)){
				xhjLpdanyuan.setDzid(Integer.parseInt(dzid));
			}
			xhjLpdanyuan.setDyName(dyName);
			xhjLpdanyuan.setPrefix(prefix);
			xhjLpdanyuan.setStart(start);
			xhjLpdanyuan.setEnd(end);
			xhjLpdanyuan.setSuffix(suffix);
			xhjLpdanyuan.setType(type);
			if(dType1!=null&&!"".equals(dType1)){
				xhjLpdanyuan.setDtype(Integer.parseInt(dType1));
			}
			if(topNum1!=null&&!"".equals(topNum1)){
				xhjLpdanyuan.setTopNum(Integer.parseInt(topNum1));
			}
			if(underNum1!=null&&!"".equals(underNum1)){
				xhjLpdanyuan.setUnderNum(Integer.parseInt(underNum1));
			}
			if(totalNum!=null&&!"".equals(totalNum)){
				xhjLpdanyuan.setTotalNum(Integer.parseInt(totalNum));
			}
			xhjLpdanyuan.setDihu(dihu);
			xhjLpdanyuan.setRemarks(remarks1);
	...
	public String updDany(){
		...
		try {
			String dzid = this.getRequest().getParameter("dzid");
			String lpdyid = this.getRequest().getParameter("lpdyid");
			String dyName = this.getRequest().getParameter("dyName");
			String lpid = this.getRequest().getParameter("lpid");
			String dType1 = this.getRequest().getParameter("dType1");
			String topNum1 = this.getRequest().getParameter("topNum1");
			...
			String remarks1 = this.getRequest().getParameter("remarks1");
			XhjLpdanyuan xhjLpdanyuan = new XhjLpdanyuan();
			if(lpdyid!=null&&!"".equals(lpdyid)){
				xhjLpdanyuan.setId(Integer.parseInt(lpdyid));
			}
			if(dzid!=null&&!"".equals(dzid)){
				xhjLpdanyuan.setDzid(Integer.parseInt(dzid));
			}
	...
	public String saveFanghao(){
		...
		try {
			...
			int userid = SecurityUserHolder.getCurrentUserLogin().getUserProfile().getId();
			XhjLpfanghao xhjLpfanghao = new XhjLpfanghao();
			if(lpid!=null&&!"".equals(lpid)){
				xhjLpfanghao.setLpid(Integer.parseInt(lpid));
			}
			if(dyid!=null&&!"".equals(dyid)){
				xhjLpfanghao.setDyId(Integer.parseInt(dyid));
			}
			if(leixing!=null&&!"".equals(leixing)){
				xhjLpfanghao.setLeixing(Integer.parseInt(leixing));
			}
			
			if(ceng!=null&&!"".equals(ceng)){
				xhjLpfanghao.setCeng(Integer.parseInt(ceng));
			}
			xhjLpfanghao.setFangHao(fangHao);
			if(shi!=null&&!"".equals(shi)){
				xhjLpfanghao.setShi(Integer.parseInt(shi));
			}
			if(ting!=null&&!"".equals(ting)){
				xhjLpfanghao.setTing(Integer.parseInt(ting));
			}
			
			if(chu!=null&&!"".equals(chu)){
				xhjLpfanghao.setChu(Integer.parseInt(chu));
			}
			if(wei!=null&&!"".equals(wei)){
				xhjLpfanghao.setWei(Integer.parseInt(wei));
			}
			if(leibei2!=null&&!"".equals(leibei2)){
				xhjLpfanghao.setLeixing(Integer.parseInt(leibei2));
			}
			if(chaoXiang!=null&&!"".equals(chaoXiang)){
				xhjLpfanghao.setChaoXiang(Integer.parseInt(chaoXiang));
			}
			
			if(decorationStandard!=null&&!"".equals(decorationStandard)){
				xhjLpfanghao.setDecorationStandard(Integer.parseInt(decorationStandard));
			}
			if(cqmj!=null&&!"".equals(cqmj)){
				xhjLpfanghao.setCqmj(Double.parseDouble(cqmj));
			}
			if(tnmj!=null&&!"".equals(tnmj)){
				xhjLpfanghao.setTnmj(Double.parseDouble(tnmj));
			}
			xhjLpfanghao.setBeiZhu(remark2);
			if(totalFloor!=null&&!"".equals(totalFloor)){
				xhjLpfanghao.setTotalFloor(Integer.parseInt(totalFloor));
			}
	...
	public String doUpdateFanghao(){
		...
		try {
			...
			String totalFloor= this.getRequest().getParameter("totalFloor");
			XhjLpfanghao xhjLpfanghao = new XhjLpfanghao();
			if(fanghId!=null&&!"".equals(fanghId)){
				xhjLpfanghao.setId(Integer.parseInt(fanghId));
			}
			xhjLpfanghao.setFhName(fhName);
			if(lpid!=null&&!"".equals(lpid)){
				xhjLpfanghao.setLpid(Integer.parseInt(lpid));
			}
			if(dyid!=null&&!"".equals(dyid)){
