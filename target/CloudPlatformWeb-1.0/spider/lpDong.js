	...
	$.ajax({
		...
		success: function(result){
			...
			$.each(result, function(i,n){
				pHtml += '<tr onclick="dongTrClicked(this,\''+n.id+'\')">'
						+ '<td class="text-center"  style="padding:0;padding-bottom:3px;padding-top:3px"><input type="checkbox" class="lpLdong" name="" value="'+n.id+'" />'
						+ '</td>'
					  + '<td  style="padding:0;padding-bottom:3px;padding-top:3px;" class="text-center">'+(i+1)+'</td><td  style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" ><a  name="lpname" >'+n.lpdName+'</a></td>'
					  + '<td  style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center"><a href="#" class="pr10" data-toggle="modal" data-id="'+n.id
					  + '" onclick="showUpdateDongDlg(this)" data-target="#Xiugai">修改</a>'
					  + '<a href="#" onclick="deleteDong('+n.id+')">删除</a></td></tr>';
			});
...
function queryDong(){
	...
	$("#tblLpdong tbody tr").each(function(i,n){
		if(lpdName=="" || $(n).find("td:eq(2)").text().indexOf(lpdName)>-1){
			$(n).show();
		} else {
			$(n).hide();
		}
...
//点击栋座某一行显示其下的单元信息
 function dongTrClicked(theLink,id){
	 ...
	 curCrrHouse = "";
 	  ...
 	  var theRow = $(theLink);
 	  theRow.css("background-color","#CCFFFF");
 	  curDongId = id;
 	  showDanyuanOfDong(curDongId);
 };
...
//						<a href="javascript:void(0)" name="lpname" onclick="dongTrClicked(this,\''+n.id+'\')">'+n.lpdName+'</a>
					sHtml += '<tr onclick="danyuanTrClicked(this,\''+n.id+'\')">' 
							+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center"><input  type="checkbox" class="lpdanyuan" name="" value="'+n.id+'" />'
							+ '</td>'
							+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" class="col-md-1">'+(i+1)+'</td>' 
							+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" ><a name="dyName">'+n.dyName+'</a></td>' 
							+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center"><a href="javascript:void(0)" class="pr10" data-toggle="modal" data-id="'+n.id+ '" onclick="showUpdateDyDlg(this)" data-target="#XiugaiDy">修改</a>'
							+ '<a href="#"  onclick="deleteDanyuan(\''+n.id+'\')">删除</a></td>' + 
						'</tr>';
				});
		...
		success: function(cengList){
			...
			if(cengList==null || cengList.length==0){
				sHtml += '<tr class="cengTr">' + 
					'<td style="padding:0;padding-bottom:3px;padding-top:3px" class="col-md-3" class="text-center">&nbsp;</td>' + 
					'<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center">暂无数据,请先新增对应房号!</td>' 
				'</tr>';
			} else {
				$.each(cengList, function(i,n){
					sHtml += '<tr class="cengTr" onclick="cengTrClicked(this,\''+n.ceng+'\')">' + 
							'<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" class="col-md-3">'+(i+1)+'</td>' + 
							//单元ID和层数
							'<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" >'+n.ceng+'</td>' 
						'</tr>';
				});
			}
		...
		success : function(fhList) {
			...
			$.each(fhList,function(i, n) {
				sHtml += '<tr onclick="fanghaoTrClicked(this,\''+n.id+'\')">'
					+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center"><input type="checkbox" class="fanghaoCB" name="" value="'+n.id+'" />'
					+ '</td>'
					+'<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center">'+ (i + 1) + "</td>"
					+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center" ><a href="javascript:void(0)">'
					+ n.fhName
					+ '</a></td>'
					+ '<td style="padding:0;padding-bottom:3px;padding-top:3px" class="text-center"><a class="pr10" onclick="updateFanghao(this,\''+n.id+'\')">修改</a>'
					+ '<a href="#"  onclick="deleteFanghao('+n.id+',this)">删除</a></td>'
					+ '</tr>';
			});
...
//		var blPutFloorNumToName = $("#chxPutFloorNumToName").prop("checked");
		...
		for(var i=iFloorStart; i<=iFloorEnd; i++){
			sHtml += '<tr><td><span style="padding:0 2px;">' + i + '</span></td>';
			if(isNaN(fhStart)&&isNaN(fhEnd)){
				for(var j=iStart;j<=iEnd;j++){
					...
					sHtml += '<td><span>' + ""+fhPrefix+i+zero+String.fromCharCode(j)+fhSuffix + '</span></td>';
				}
			}else{
				for(var j=iStart;j<=iEnd;j++){
					zero = "";
					...
					sHtml += '<td><span>' + ""+fhPrefix+i+zero+j+fhSuffix + '</span></td>';
				}
			}
