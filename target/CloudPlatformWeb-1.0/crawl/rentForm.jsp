							...
							<div class="panel-body">
									...
									<div class="col-sm-12">
										<div class="form-group">
											...
											<div class="col-sm-2">
												<select class="form-control sboxit-2" name="paywar" id="paywar">
													<option value="1">面议</option>
													<option value="2" selected="selected">押一付三</option>
													<option value="3">押一付二</option>
													<option value="4">押一付一</option>
													<option value="6">押二付三</option>
													<option value="5">押二付二</option>
													<option value="9">押二付一</option>
													<option value="8">年付</option>
									...
									<div class="col-sm-12">
										<div class="form-group">
											<label class="col-sm-2 control-label left" >租赁方式：</label>
											<div class="col-sm-8">
												<label class="radio-inline">
													<input type="radio" name="rentType" value="1" checked="true"><label class="radioLabel">整租</label>
													<input type="radio" name="rentType" value="2"><label class="radioLabel">合租</label>
												</label>
											</div>
										</div>
									</div>
								...
								<div class="panel-body">
									...
									<div class="col-sm-12">
										<div class="form-group">
											...
											<div class="col-sm-8" id="ptssDiv">
												<label class="checkbox-inline">
													<input type="checkbox" value="1" name="ptss"><label class="radioLabel">床</label>
													<input type="checkbox" value="2" name="ptss"><label class="radioLabel">衣柜</label>
													<input type="checkbox" value="3" name="ptss"><label class="radioLabel">沙发</label>
													<input type="checkbox" value="4" name="ptss"><label class="radioLabel">电视</label>
													<input type="checkbox" value="5" name="ptss"><label class="radioLabel">冰箱</label>
													<input type="checkbox" value="6" name="ptss"><label class="radioLabel">洗衣机</label>
													<input type="checkbox" value="7" name="ptss"><label class="radioLabel">空调</label>
													<input type="checkbox" value="8" name="ptss"><label class="radioLabel">热水器</label>
													<input type="checkbox" value="9" name="ptss"><label class="radioLabel">宽带</label>
													<input type="checkbox" value="10" name="ptss"><label class="radioLabel">暖气</label>
													<button type="button" class="btnMultiselect" onclick="checkAll('ptss',true);">全选</button>
													<button type="button" class="btnMultiselect" onclick="checkAll('ptss',false);">取消</button>
												</label>
											</div>
										</div>
									</div>
...
function initMiaoLu(){
	...
	if(obj.ptss != null && obj.ptss != ""){
		$.each(obj.ptss.split(','),function(i,n){
			$("#postform input[name='ptss']").filter(function () { return $(this).next().text() == n; }).attr("checked", true);
		})
	}
	lpName = obj.lpName;
	if(lpName == ""){
...
function initShouCangMiaoLu(obj){
	...
	$("#paywar").find("option[value='"+obj.paywar+"']").attr("selected",true);
	$("#postform input[name='rentType'][value='"+obj.rentType+"']").attr("checked","checked");
	$("#cover").val(obj.cover);
	if(obj.ptss != null && obj.ptss != ""){
		$.each(obj.ptss.split(','),function(i,n){
			$("#postform input[name='ptss']").filter(function () { return $(this).val() == n; }).attr("checked", true);
		})
	}
...
function save(id){
	...
	var fangYuanChuZuRelease=new Object();
	fangYuanChuZuRelease.id="${param.scId}";
	fangYuanChuZuRelease.businessType="fangyuan_chuzu";
	fangYuanChuZuRelease.type=type;
	fangYuanChuZuRelease.status=id;
	fangYuanChuZuRelease.employeeId=uid;
	fangYuanChuZuRelease.crawlId="<%=id%>";
	fangYuanChuZuRelease.currentUserName="<%=SecurityUserHolder.getCurrentUserLogin().getUserLogin().getUsername()%>";
	if(fangYuanChuZuRelease.crawlId =="null"){
		fangYuanChuZuRelease.crawlId=null;
	}
	if("${param.houseNumber}" != ""){
		fangYuanChuZuRelease.houseNumber = "${param.houseNumber}";
	}
	fangYuanChuZuRelease.lpName=lpName;
	fangYuanChuZuRelease.title=$("#title").val();
	fangYuanChuZuRelease.address=$("#address").val();
	fangYuanChuZuRelease.area=$("#area").val();
	fangYuanChuZuRelease.price=$("#price").val();
	fangYuanChuZuRelease.directionType=$("#direction").val();
	fangYuanChuZuRelease.fitmentType=$("#postform input[name='fitment']:checked").val();
	fangYuanChuZuRelease.xqid=$("#selectXiaoQu").val();
	fangYuanChuZuRelease.hx_s=$("#hx_s").val();
	fangYuanChuZuRelease.hx_t=$("#hx_t").val();
	fangYuanChuZuRelease.hx_c=$("#hx_c").val();
	fangYuanChuZuRelease.hx_w=$("#hx_w").val();
	fangYuanChuZuRelease.hx_y=$("#hx_y").val();
	fangYuanChuZuRelease.floor_c=$("#floor_c").val();
	fangYuanChuZuRelease.floor_z=$("#floor_z").val();
	fangYuanChuZuRelease.label=$("#label").val();
	fangYuanChuZuRelease.wubalabel=$("#wubalabel").val();
	fangYuanChuZuRelease.houseType=$("#postform input[name='houseType']:checked").val();
	fangYuanChuZuRelease.propertyAge=$("#postform input[name='propertyAge']:checked").val();
	fangYuanChuZuRelease.summary=$("#summary").val();
	fangYuanChuZuRelease.pstatus = id;
	var label = $("#label").val();
	...
	fangYuanChuZuRelease.label=label;
	fangYuanChuZuRelease.wubalabel=wubalabel;
	fangYuanChuZuRelease.yijuhuaguanggao = $("#yijuhuaguanggao").val();
	fangYuanChuZuRelease.houseNumber = $("#houseNumber").val();
//	fangYuanChuZuRelease.type = $("#houseType").val();
	fangYuanChuZuRelease.houseId = "${param.saleOrRentId}";
	fangYuanChuZuRelease.cityId = getCityName();
	fangYuanChuZuRelease.provinceId = getProvinceName();
	fangYuanChuZuRelease.qyId = $("#qyId option:selected").text();
	fangYuanChuZuRelease.sqId = $("#sqId option:selected").text();
	fangYuanChuZuRelease.paywar = $("#paywar").val();
	fangYuanChuZuRelease.rentType = $("#postform input[name='rentType']:checked").val();
	fangYuanChuZuRelease.personalFangYuan = $("#personalFangYuan").val();
	fangYuanChuZuRelease.cover = $("#cover").val();

	var ptssArray = [];
	$("#postform input[name='ptss']:checked").each(function(){
		ptssArray.push($(this).val());
	});
	fangYuanChuZuRelease.ptss = ptssArray.join(",");
	...
	if(id == 0){
		var flag = validatePicNum(fangYuanChuZuRelease.houseNumber,array,array1,array2);
		if(!flag){
			return;
		}
	}
	...
	fangYuanChuZuRelease.s_picture = array.join(",");
	fangYuanChuZuRelease.f_picture = array1.join(",");
	fangYuanChuZuRelease.x_picture = array2.join(",");
	var jsonData=JSON.stringify(fangYuanChuZuRelease);
	$.ajax({
		url: cloudPlatformUrl+"/services/newenv/lpzd/crawl/saveFangYuanChuZuRelease/",
		dataType: "json", 
		type: "POST", 
		...
		success : function(result){
			...
			if(result != null){
				if(result.houseNumber == null || result.houseNumber == ""){
					window.location.href = "<%=basePath%>/crawl/rentManagment.jsp?from="+id+"&id="+result.id+"&hType="+paramType;
				}else{
					window.location.href = "<%=basePath%>/crawl/rentManagment.jsp?from="+id+"&id="+result.id+"&houseNumber="+result.houseNumber+"&hType="+paramType;
				}
			}else{
...
function validateEmpty(){
	...
	if(floor_c == "" || floor_z == "" ){
		...
		flag = false;
		bootbox.alert({title:"提示",message:"面积不能为空 !"});
	}else if(price == ""){
		flag = false;
...
function checkAll(checkBoxName,state){
