												...
												<div class="tab-pane active" id="tab-07">
													<div class="panel">
														...
														<span class="input-group-btn pull-left">
															<a id="xinzengfangyuan" href="saleForm.jsp?from=3&hType=0"></a>
...
<script type="text/html" id="publishTemplate">
	...
	<td class="middle-align" data-priority="1" colspan="1" data-columns="publishTable-col-2">
...
<td class="middle-align text-center" data-priority="2" colspan="1" data-columns="shouCangTable-col-5">
	<div class="line24" data-content="source"></div>
</td>
...
function initXinzengFangyuan(hType){
	$('#xinzengfangyuan').attr('href','saleForm.jsp?from=3&hType='+hType);
}

function selectStatus(thiz){
	var id = $(thiz).attr("data-val");
	$("#fromlist input[name='status']").val(id);
	selectStyle(thiz);
	filterAll();
	initShouCang(); 
	cancelAllCheck();
...
function initShouCang(pageNum){
	...
	$.addTemplateFormatter({
	    ...
	    shouFormatter : function(value, template) {
	    	...
	    	var url = "<%=basePath%>/crawl/saleForm.jsp?type=fangyuan_chushou&id="+value+"&hType="+businessType;
	    ...
	    huanyuanFormatter : function(value,ucCause,template) {
	...
	if(status == '0'){
		for(var i = 0 ; i < len; i++){
			if(resultObj.gridModel[i].crawlId == '0'){
				var updateBMSUrl = "<%=basePath%>/crawl/saleForm.jsp?type=fangyuan_chushou&id="+resultObj.gridModel[i].crawlId+"&houseNumber="+resultObj.gridModel[i].housenumber+"&hType="+ businessType;
...
function shangjia(status){
	...
	if(status == '0'){
		...
		$.addTemplateFormatter({
		    ...
		    shouFormatter : function(value, template) {
		    	var url = "<%=basePath%>/crawl/saleForm.jsp?type=fangyuan_chushou&id="+value;
		    	return '<a href="'+url+'" class="btn btn-secondary btn-xs">修改</a>';
		    },
		    deleteShouFormatter : function(value, template) {
		    	return '<a href="javascript:void(0);" class="btn btn-secondary btn-xs" style="margin-top: 2px;" onclick="openShouCang('+value+');">删除</a>';
		...
		bean.uids=uids;
		bean.rids=rids;
		bean.businessType = "fangyuan_chushou";
		obj.bean=bean;
		...
		var result = $.ajax({
			...
			success : function(resultObj){
				if(resultObj.status == "error"){
					...
					var jumpHref = "";
					if(null != resultObj.fangYuanChuShouRelease.houseNumber || "" != resultObj.fangYuanChuShouRelease.houseNumber){
						jumpHref = "saleForm.jsp?type=fangyuan_chushou&id=0&houseNumber="+resultObj.fangYuanChuShouRelease.houseNumber+"&hType="+resultObj.fangYuanChuShouRelease.type+"&scId="+resultObj.fangYuanChuShouRelease.id;
					}
					if(null != resultObj.fangYuanChuShouRelease.crawlId || "" != resultObj.fangYuanChuShouRelease.crawlId){
						jumpHref = "saleForm.jsp?type=fangyuan_chushou&id="+resultObj.fangYuanChuShouRelease.crawlId+"&hType="+resultObj.fangYuanChuShouRelease.type;
					}
					if(null != resultObj.fangYuanChuShouRelease.crawlId || "" != resultObj.fangYuanChuShouRelease.crawlId){
						jumpHref = "saleForm.jsp?type=fangyuan_chushou&id="+resultObj.fangYuanChuShouRelease.crawlId+"&hType="+resultObj.fangYuanChuShouRelease.type+"&scId="+resultObj.fangYuanChuShouRelease.id;
					}
...
function golistMySale(){
	window.location.href = BMSUrl+'/pages/house/listMySale.action?clickedTopMenuId=32';
}
