...
<html lang="en">
	...
	<head>
		...
		<script src="<%=basePath%>/js/base/manager.js"></script>
			...
			<div class="main-content">
				...
				<div class="page-title">
					<div class="breadcrumb-env pull-left">
						<ol class="breadcrumb bc-1">
							...
							<li class="active">
								<strong><a href="<%=basePath%>/base/regionManager.jsp?k=base&p=dq">地区管理</a></strong>
							</li>
						</ol>
					</div>
				</div>
				...
				<div class="panel panel-default">
						<div class="input-group col-lg-3" style="padding-bottom: 10px;" >
							<input type="text" class="form-control no-right-border form-focus-purple"  placeholder="请输入商圈名称" id="sqUserName">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="scrollable ps-container" data-max-height="400" style="max-height: 400px;">
				<form role="form" class="form-horizontal">
					...
					<div class="form-group">
							...
							<div class="col-md-5">
								<input type="text" class="col-xs-6 wenbenkuan1" id="xxzbx2" name="x" readonly>
								<input type="text" class="col-xs-6 wenbenkuan1" id="xxzby2" name="y" readonly>
							</div>
							<input type="hidden" class="form-control" id="address" name="condition.xhjLpschool.address" >
							<a class="btn btn-secondary" id="loadXY2"  data-toggle="modal" data-target="#map-info-windom">获取位置</a>
					</div>
				</form>
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-danger" data-dismiss="modal" onclick="coutnridEmpty11()">关闭</button>
			</div>
		</div>
	</div>
</div>
			...
			<div class="scrollable ps-container" data-max-height="400" style="max-height: 400px;">
					<table class="table">
			            <tbody>
			              ...
			              <tr>
								<td  width="120px"><span style="color:red">坐标地址:</span></td>
								<td>
									<input type="text" class="col-xs-6 wenbenkuan1" id="xxzbx3" name="x" readonly>
									<input type="text" class="col-xs-6 wenbenkuan1" id="xxzby3" name="y" readonly>
								</td>
								<td>
								<input type="hidden" class="form-control" id="address" name="condition.xhjLpschool.address" >
								<a class="btn btn-secondary" id="loadXY3"  data-toggle="modal" data-target="#map-info-windom">获取位置</a>
								</td>
						</tr>
		...
		<div class="modal-content">
			...
			<div class="scrollable ps-container" data-max-height="400" style="max-height: 400px;">
				<form role="form" class="form-horizontal" name="pageForm" id="pageForm">
					<h4><b>站点基本信息</b></h4>
					<div class="form-group-separator" style="display: none"></div>
					...
					<div class="form-group">
						<label class="col-sm-2 control-label"><span style="color:red">坐标地址：</span></label>
						<div class="col-sm-5">
		...
		<div class="modal-content">
			...
			<div class="scrollable ps-container" data-max-height="400" style="max-height: 400px;">
				<form role="form" class="form-horizontal">
					...
					<div class="form-group-separator"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label">修改商圈：</label>
						<div class="col-sm-9">
...
function buildCountry(){
	$.ajax({ 
		...
		success: function(data){
			...
			dHtml += '<button class="btn btn-secondary btn-xs" data-toggle="modal"   type="button" onclick="bainji()"><i class="fa-pencil"></i> 编辑</button>';
			dHtml += '<button class="btn btn-success btn-xs" data-toggle="modal" data-target="#guojia-zeng"><i class="fa-plus"></i> 新增</button>';
			dHtml += '</div>';
			$("#country").html(cHtml);
...
function buildPro(val){
	...
	$.ajax({ 
		...
		success: function(data){
			...
			var dHtml = '<div class="pull-right">';
			dHtml += '<button class="btn btn-secondary btn-xs" data-toggle="modal"tybe ="button" onclick="provincebiaji()"><i class="fa-pencil"></i> 编辑</button>';
			dHtml += '<button class="btn btn-success btn-xs" data-toggle="modal" data-target="#sheng-zeng"><i class="fa-plus"></i> 新增</button>';
			dHtml += '</div>';
			$("#pro").html(pHtml);
...
function buildCity(val){
	...
	$.ajax({ 
		...
		success: function(data){
			...
			var dHtml = '<div class="pull-right">';
			dHtml += '<button class="btn btn-secondary btn-xs" data-toggle="modal" tybe="button" onclick="citybiaji()"><i class="fa-pencil"></i> 编辑</button>';
			dHtml += '<button class="btn btn-success btn-xs" data-toggle="modal" data-target="#cheng-zeng"><i class="fa-plus"></i> 新增</button>';
			dHtml += '</div>';
			$("#city").html(cHtml);
...
success: function(data){
			...
			var dHtml = '<div class="pull-right">';
			dHtml += '<button class="btn btn-secondary btn-xs" data-toggle="modal"tybe="button" onclick="biajiArea()"><i class="fa-pencil"></i> 编辑</button>';
			dHtml += '<button class="btn btn-success btn-xs" data-toggle="modal" data-target="#qu-zeng"><i class="fa-plus"></i> 新增</button>';
			dHtml += '</div>';
			$("#sQy").html(cHtml);
...
$.ajax({
	...
	type: "POST",
	async : false,
	data :{"qyName":qyid,"cityID":cityid,"cnameCoding":$("#qyNameData").val(),"x":x2,"y":y2},
	success: function(data){
	var cHtml = '';
	if(data != null){
		alert('添加成功!');
...
$.ajax({
		...
		type: "POST",
		async : false,
		data :{"qyName":qyName,"qyID":qyId,"cnameCoding":$("#qyDataName").val(),"x":x3,"y":y3},
		success: function(data){
		if(data>0){
				alert('修改成功!');
 ...
 * @return
 */
function selectData(cityName,qyName,sqName,sqid,x,y){
...
url: "<%=basePath%>/base/regionManager!updateData.action",
		...
		type: "POST",
		async : false,
		data :{"sqname":shangquanName,"sqID":shangquanID,"x":x1,"y":y1},
		success: function(data){
			var cHtml = '';
			if(data>0){
				alert('修改成功!');
...
var x = $("#xxzbx").val();
var y =  $("#xxzby").val();
if(shangQuanName != null && shangQuanName !='' && shangQuanName != 'null'){
if(x !=null && x !='' && x != 'null'){
if(fian){
$.ajax({
		...
		type: "POST",
		async : false,
		data :{"qyID":quyuid,"sqname":shangQuanName,"x":x,"y":y},
		success: function(data){
			if(data != null){
				alert('添加成功!');
...
$.ajax({
		...
		var cHtml = '';
			...
			if(n.x != null && '' != n.x && "null" !=n.x)
			{
				$("#xxzbx3").val(n.x);
			}
			if(n.y !=null && ''!=n.y && "null" !=n.y)
			{
				$("#xxzby3").val(n.y);
			}
			});
...
$("#loadXY2").click(function(){
		$("#up").val("2");
});
$("#loadXY3").click(function(){
		$("#up").val("3");
});
